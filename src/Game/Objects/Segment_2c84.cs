...
namespace OpenCiv1
{
	public class Segment_2c84
	{
		...
		public void F0_2c84_0000_ShowTopMenu(short playerID, short unitID, short menuIndex)
		{
			...
			// function body
			this.oCPU.WriteInt16(this.oCPU.DS.UInt16, 0xd4ca, -1);
			this.Var_654a = 0;
		...
		private void F0_2c84_00ad_GameMenu()
		{
			...
			if (oParent.GameData.TurnCount == 0)
			{
				...
				this.oCPU.WriteUInt16(this.oCPU.DS.UInt16, 0xb276, 0x10);
			}

			// Instruction address 0x2c84:0x00c8, size: 5
			this.oParent.CAPI.strcpy(0xba06, " Tax Rate\n Luxuries Rate\n FindCity\n Options\n Save Game\n REVOLUTION!\n \n Retire\n QUIT to DOS\n");

			if (((ushort)this.oParent.GameData.SpaceshipFlags & 0x100) != 0)
			{
				// Instruction address 0x2c84:0x00e0, size: 5
				this.oParent.CAPI.strcat(0xba06, " View Replay\n");
			}
			...
			switch (selectedOption)
			{
				...
				case 3: // Options
					{
						...
						do
						{
							...
							if (this.oCPU.ReadUInt16(this.oCPU.DS.UInt16, 0x2f9c) == 0)
							{
								oParent.GameData.GameSettingFlags.Value ^= (short)(1 << index);
							}

							if (index != -1)
							{
								this.oCPU.WriteInt16(this.oCPU.DS.UInt16, 0x2f9a, index);
							}
						}
						while (index != -1 || this.oCPU.ReadUInt16(this.oCPU.DS.UInt16, 0x2f9c) != 0);
				...
				case 5: // Revolution
					this.oCPU.WriteInt16(this.oCPU.DS.UInt16, 0xd4ca, -2);
					break;
		...
		private void F0_2c84_01d8_OrdersMenu(short playerID, short unitID)
		{
			...
			this.oParent.CAPI.strcat(0xba06, " No Orders \x008fspace\n");
			orders[orderCount++] = ' ';

			// All orders are enabled by default
			this.oCPU.WriteUInt16(this.oCPU.DS.UInt16, 0xb276, 0x0);
			...
			if (unit.TypeID == (short)UnitTypeEnum.Settlers)
			{
				if (improvements.HasFlag(TerrainImprovementFlagsEnum.City))
				{
					...
					this.oParent.CAPI.strcat(0xba06, " Add to City \x008fb\n");
				}
				else
				{
					// Instruction address 0x2c84:0x027a, size: 5
					this.oParent.CAPI.strcat(0xba06, " Found New City \x008fb\n");
				}
				...
				if (!improvements.HasFlag(TerrainImprovementFlagsEnum.Road))
				{
					...
					this.oParent.CAPI.strcat(0xba06, " Build Road \x008fr\n");
					orders[orderCount++] = 'r';
				}
				else
				{
					...
					if (!improvements.HasFlag(TerrainImprovementFlagsEnum.RailRoad) && this.oParent.Segment_1ade.F0_1ade_22b5_PlayerHasTechnology(playerID, TechnologyEnum.Railroad))
					{
						...
						this.oParent.CAPI.strcat(0xba06, " Build RailRoad \x008fr\n");
						orders[orderCount++] = 'r';
					}
				}
				...
				if (!improvements.HasFlag(TerrainImprovementFlagsEnum.Irrigation))
				{
					...
					{
						if (this.oParent.GameData.TerrainModifications[(int)terrainType].IrrigationEffect >= 0)
						{
							// Instruction address 0x2c84:0x0342, size: 5
							this.oParent.CAPI.strcat(0xba06, " Change to ");
					...
					if (this.oParent.GameData.TerrainModifications[(int)terrainType].IrrigationEffect != -1)
					{
						// Instruction address 0x2c84:0x0386, size: 5
						this.oParent.CAPI.strcat(0xba06, " \x008fi\n");
						orders[orderCount++] = 'i';
					}
				}
				...
				{
					if (this.oParent.GameData.TerrainModifications[(int)terrainType].MiningEffect <= -2)
					{
						// Instruction address 0x2c84:0x03dc, size: 5
						this.oParent.CAPI.strcat(0xba06, " Build Mines");
					}
					else if (this.oParent.GameData.TerrainModifications[(int)terrainType].MiningEffect >= 0)
					{
					...
					if (this.oParent.GameData.TerrainModifications[(int)terrainType].MiningEffect != -1)
					{
						...
						this.oParent.CAPI.strcat(0xba06, " \x008fm\n");
						orders[orderCount++] = 'm';
					}
				}
				...
				if (improvements.HasFlag(TerrainImprovementFlagsEnum.Pollution))
				{
					...
					this.oParent.CAPI.strcat(0xba06, " Clean up Pollution \x008fp\n");
					orders[orderCount++] = 'p';
				}
			}
			...
			if (unit.TypeID == (short)UnitTypeEnum.Settlers)
			{
				...
				this.oParent.CAPI.strcat(0xba06, " Build Fortress \x008ff\n");
			...
			this.oParent.CAPI.strcat(0xba06, " Wait \x008fw\n Sentry \x008fs\n GoTo\n");
			...
			if (((ushort)improvements & (ushort)TerrainImprovementFlagsEnum.PillageMask) != 0 && unit.TypeID < (short)UnitTypeEnum.Diplomat && unit.TypeID != (short)UnitTypeEnum.Fighter)
			{
				...
				this.oParent.CAPI.strcat(0xba06, " Pillage \x008fP\n");
				orders[orderCount++] = 'P';
			}
			...
			if (improvements.HasFlag(TerrainImprovementFlagsEnum.City))
			{
				...
				this.oParent.CAPI.strcat(0xba06, " Home City \x008fh\n");
				orders[orderCount++] = 'h';
			}
			...
			{
				this.oCPU.WriteInt16(this.oCPU.DS.UInt16, 0xd4ca, (short)orders[selectedOrder]);
			}
		...
		private void F0_2c84_0615_AdvisorsMenu()
		{
			...
			this.oParent.CAPI.strcpy(0xba06, " City Status (F1)\n Military Advisor (F2)\n Intelligence Advisor (F3)\n Attitude Advisor (F4)\n");
		...
		private void F0_2c84_06e4_ShowWorldMenu()
		{
			...
			if ((this.oParent.GameData.SpaceshipFlags & 0xfe00) == 0)
			{
				...
				this.oCPU.WriteUInt16(this.oCPU.DS.UInt16, 0xb276, 0x20);
			}

			// Instruction address 0x2c84:0x0700, size: 5
			this.oParent.CAPI.strcpy(0xba06, " Wonders of the World (F7)\n Top 5 Cities (F8)\n Civilization Score (F9)\n");
			...
			switch (selectedOption)
			{
				...
				case 4: // Demographics
					this.oParent.WorldMap.F12_0000_0d6d_ShowsDemographicsPopup(this.oParent.GameData.HumanPlayerID);
					break;
		...
		private void F0_2c84_07af_CivilopediaMenu()
		{
			...
			this.oParent.CAPI.strcpy(0xba06, " Complete\n Civilization Advances\n City Improvements\n Military Units\n Terrain Types\n");
